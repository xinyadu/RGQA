...
class GenIEModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        return (doc_key, role, sample_output, tgt_token_ids) 
            ...
            for tup in outputs:
                for idx in range(len(tup[0])):
                    ...
                    pred = {
                        'doc_key': tup[0][idx],
                        'role': tup[1][idx],
                        'predicted': self.tokenizer.decode(tup[2][idx].squeeze(0), skip_special_tokens=True),
                        'gold': self.tokenizer.decode(tup[3][idx].squeeze(0), skip_special_tokens=True) 
                    }
